...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    if a == 0:
        assert_close(sig, 0.5)
    assert(sigmoid(a - 1.0) <= sig and sigmoid(a + 1.0) >= sig)
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    list_sums = add(sum(ls1), sum(ls2))
    summed_lists = sum(zipWith(add)(ls1, ls2))
    assert_close(list_sums, summed_lists)
